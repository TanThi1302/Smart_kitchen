...
export function ProductGrid({ products, variants }) {
  ...
  const showcaseItems = products.slice(1, 6)
  const scrollRef = useRef(null)
  const pauseRef = useRef(false)
  const showCarouselControls = showcaseItems.length > 4

  // Auto-scroll (dừng khi hover)
  useEffect(() => {
    const container = scrollRef.current
    if (!container) return

    const handleEnter = () => (pauseRef.current = true)
    const handleLeave = () => (pauseRef.current = false)
    ...
    const id = setInterval(() => {
      ...
      else if (el.scrollLeft <= 2) direction = 1
    ...
    <div className="rounded-[36px] bg-gradient-to-b from-[#030c1d] via-[#050f27] to-[#01030a] p-8 md:p-12 text-white shadow-[0_40px_120px_rgba(3,10,25,0.65)] ring-1 ring-sky-500/20">
      <div className="grid gap-10 lg:grid-cols-[1.2fr_1fr] items-center">
        ...
        >
          ...
          <div className="flex flex-wrap gap-4 text-xs text-[#031226]">
            <Link
              to="/products"
              className="rounded-full inline-flex items-center justify-center bg-sky-300 px-6 py-3 text-sm font-semibold shadow-[0_15px_40px_rgba(56,189,248,0.45)] transition hover:bg-sky-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-white/80"
            >
              Khám phá ngay
            </Link>
      ...
      <div className="mt-12 space-y-5 text-center">
        ...
        <h4 className="text-3xl font-bold text-white">
          Sản phẩm thế hệ mới đến từ <span className="text-sky-300">Smart Kitchen</span>
        </h4>
      </div>
        ...
        <div className="relative mt-10">
          {showCarouselControls && (
            <button
              ...
              className="absolute left-2 top-1/2 z-20 hidden -translate-y-1/2
                         md:flex items-center justify-center
                         h-10 w-10 rounded-full
                         bg-white/10 backdrop-blur
                         ring-1 ring-white/25
                         text-sky-50
                         shadow-[0_10px_30px_rgba(3,10,25,0.6)]
                         transition transform hover:scale-105 hover:bg-white/15
          ...
          <div
            ref={scrollRef}
            className="flex gap-6 overflow-x-auto scroll-smooth pb-3 snap-x snap-mandatory px-3"
          >
            {showcaseItems.map((item, idx) => (
              <motion.article
                ...
                className="group relative w-72 shrink-0 snap-center overflow-hidden rounded-[28px] bg-gradient-to-b from-[#051733] via-[#040a16] to-[#010309] p-0 text-left shadow-[0_25px_70px_rgba(3,10,25,0.6)] ring-1 ring-sky-500/15"
              >
                <Link
                  to={item.slug ? `/products/${item.slug}` : '#'}
                  className={`block h-full p-5 focus:outline-none focus-visible:ring-2 focus-visible:ring-white/40 ${
                ...
                >
                  ...
                  <div className="mt-5 space-y-2">
                    <p className="text-xs font-semibold uppercase tracking-[0.35em] text-sky-200">Sản phẩm mới</p>
                  ...
                  {item.oldPrice && <p className="text-xs text-white/60 line-through">{item.oldPrice}</p>}
                </Link>
              </motion.article>
            ))}
          </div>
          ...
          {showCarouselControls && (
            <button
              ...
              onClick={() => scrollBy(280)}
              className="absolute right-2 top-1/2 z-20 hidden -translate-y-1/2
                         md:flex items-center justify-center
                         h-10 w-10 rounded-full
                         bg-white/10 backdrop-blur
                         ring-1 ring-white/25
